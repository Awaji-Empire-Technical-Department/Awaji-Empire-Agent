...
2026年8月分の `#配信コメント` 月次リセットがフォールバックcron含め完全にスキップされたインシデントを受け、再発防止策を適用。

### Fixed

- **`fallback_reset` のスキップが無音だった問題**: 条件不一致で早期returnする際にログが一切出力されず、ネットワーク障害等で発火を逃してもインシデント検知が翌月までずれ込んでいた（2026-08-21分は09-08まで未検知）。発火時・スキップ時双方でログを出力するようにした（`_try_monthly_reset` の冪等スキップも同様）
- **`should_fallback_run()` の `now.hour == 6` 二重ガードを削除**: `tasks.loop(time=...)` が既に06:00 JST発火を保証しており、実行タイミングが多少ずれた場合に無音で条件不一致になるリスクがあった。時刻判定はloop側に一任
- **`now.day == 21` の単日縛りを `now.day >= 21` に変更**: 当日にネットワーク障害等で実行を逃すと翌月21日まで補完機会がなかった問題を修正し、当月末までの間、未リセットなら翌日以降のループ発火で補完できるようにした

---

## [1.8.3] - 2026-08-24

`#配信コメント` チャンネルの月次自動リセットが機能していなかった不具合を修正。

### Fixed

- **フォールバックcronがJST壁時計に同期していなかった問題**: `stream_comment_reset` の `fallback_reset` 定時タスクが `@tasks.loop(hours=24)`（Bot起動時刻起点）で実装されており、内部の「毎月21日06:00 JST」判定がBotをその時間帯に再起動しない限り恒久的に成立しなかった。`mass_mute` と同じ `@tasks.loop(time=...)` パターンに変更し、壁時計に同期させた
- **Bot再起動時の冪等性状態消失**: 当月リセット済みかの判定がインメモリ変数のみに依存しており、Bot再起動のたびに消失していた（既存のDB照会APIが未使用だった）。Cogロード時にDBを照会して状態を復元するようにし、再起動直後の二重リセットを防止

---

## [1.8.2] - 2026-07-23

セキュリティレビューで発見した Webダッシュボードの脆弱性を修正。設計記録: `docs/adr/027`

### Security

- **格納型XSS の修正**（ADR-027）: 順位表・称号表を `fetch()` の生JSONから `innerHTML` へ描画する際、ユーザー制御値（Discord 表示名・称号名）を未エスケープで埋め込んでいた問題を修正。`tournament.js` / `lounge.js` / `dashboard_titles.js` の該当箇所へ `escapeHtml` を適用し、削除ボタンは属性へのユーザー値埋め込みを排除

### Changed

- **セッション署名鍵 `SECRET_KEY` の必須化（破壊的変更）**（ADR-027）: 既知の固定値へのフォールバックを廃し、未設定なら起動失敗（fail-closed）とした。Cookie 偽造による管理者なりすまし（認証バイパス）を構造的に防止。**`SECRET_KEY` 未設定の既存デプロイは webapp が起動しなくなるため、更新前に環境変数を設定すること**（`discord_bot/.env.example` 参照）

---

## [1.8.1] - 2026-06-14

ギルド未加入者がフォーム回答後にダッシュボードへ流入できる不具合を修正。設計記録: `docs/adr/026`

### Fixed

- **ギルド未加入者のダッシュボード流入**（ADR-026）: ADR-024 の回答フロー緩和の副作用で、未加入者がフォーム回答後に「ホームへ戻る」やURL直打ちでダッシュボードへアクセスできていた問題を修正。ログイン時に加入状態を `session['is_guild_member']` に保持し、ダッシュボード(`index`)で未加入者を 403 で拒否（サーバーガード）。回答完了画面の「ホームへ戻る」ボタンも未加入者向けに無効化
