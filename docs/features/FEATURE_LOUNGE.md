...
- **ステータス**: 設計中（Phase 3 仕様変更中）
- **作成日**: 2026-05-07
- **ADR**: [ADR-013](./adr/013-lounge-system.md)
- **運営サーバ**: 社畜天狗Discord サーバ「淡路帝国」

> [!WARNING]
> **仕様変更 (2026-05-23): レースごとの申告方式 → セッション最終順位申告方式へ移行**
> 詳細は §10 を参照。実装計画は `tasks/todo.md` Phase 3 を参照。
...
- 2v/3v時のチーム数上限
- MMRデルタの算出式（順位ごとの増減値設計 → Phase 3 着手前に確定が必要）
- セッション終了時に未申告者が存在した場合の扱い（強制最下位 or 除外）

---

## 10. 仕様変更: セッション最終順位申告方式（2026-05-23）

### 変更の背景

参加型配信という運用特性上、レースごとに全員が申告する方式はゲームの流れを断絶させる。  
12レースが終わった後に最終順位を1回だけ申告し、MMRと称号を表示するフローに変更する。
...
[ホストがセッション終了操作]
      ↓
[各プレイヤーが最終順位（1〜24位）を1回申告]
      ↓
[全員申告完了（またはホストが強制終了）]
      ↓
[システムがMMRを計算・更新]
      ↓
[結果モーダル: 最終順位 / MMR増減 / 現在の称号を表示]
```

### 変更前後の対比

| 項目 | 変更前（レースごと申告） | 変更後（最終順位申告） |
|------|---------------------|------------------|
| 申告タイミング | 各レース終了後（最大12回） | セッション終了後に1回 |
| 申告内容 | そのレースの順位 + 回線落ち | セッションの最終順位（1〜24位） |
| ポイント計算 | レースごとに配点テーブルで変換・累積 | 最終順位ベースのMMRデルタに一本化 |
| 配点テーブル | 必要（§2.3） | 廃止（MMRデルタテーブルに置換） |
| 回線落ち申告 | 即時申告が必要 | 申告不要（最終順位に結果が含まれる） |
| 配信への影響 | レースごとに中断あり | 終了後に一括処理で中断なし |

### DB変更

```sql
-- 追加
CREATE TABLE lounge_session_final_scores (
    id              BIGINT AUTO_INCREMENT PRIMARY KEY,
    session_id      BIGINT NOT NULL,
    user_id         BIGINT NOT NULL,
    final_rank      TINYINT NOT NULL,       -- 1〜24
    mmr_delta       INT NOT NULL DEFAULT 0, -- 計算後に確定
    submitted_at    DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY (session_id, user_id),
    FOREIGN KEY (session_id) REFERENCES lounge_sessions(id)
);

-- lounge_race_scores は新規書き込み停止（テーブル自体は後方互換で保持）
```

### 廃止するエンドポイント

```text
POST   /lounge/races/{race_id}/scores/report   ← 廃止
POST   /lounge/races/{race_id}/disconnect       ← 廃止
PATCH  /lounge/races/{race_id}/approve          ← 廃止
```

### 追加するエンドポイント

```text
POST   /lounge/sessions/{id}/final-scores/report   最終順位申告（final_rank）
GET    /lounge/sessions/{id}/final-scores          申告状況一覧（リアルタイム確認用）
```

### MMRデルタテーブル（確定）

身内向け運用のため減点なし・全員プラス制。
...
ホストが任意のプレイヤーを「除外」操作できる。除外されたプレイヤーはMMR増加なし・称号対象外。  
想定頻度は低いが、通信障害等で申告不能になった場合のセーフティとして実装する。

### コース申告（確定）

廃止。ホストがレースごとにコース名を入力する機能は実装しない。  
`lounge_race_results` テーブルおよびコース重複検知ロジックも Phase 3 で削除対象とする。
