...
| 自己申告方式の理由 | Webアプリ側からゲーム内の結果を自動取得する手段は公式には提供されていない。強制的に取得しようとすると通信の傍受・メモリ読み取り・APIの不正利用といったハック的手法が必要となり、各ゲームの利用規約・不正競争防止法・著作権法等に抵触する可能性が高い。そのため本システムはゲーム結果の自動連携を行わず、プレイヤーの自己申告を唯一の入力経路とする |
...
| `multiplayer` | マリオカート（最大24人） | ポイント合計（レース順位積算） |
...
INSERT INTO game_titles (name, match_type, max_players, score_type) VALUES
    ('マリオカート8 デラックス', 'multiplayer', 12, 'point_sum'),  -- MKワールドを想定するなら24
    ('マリオカートワールド',    'multiplayer', 24, 'point_sum'),
    ('遊び大全',                '1v1',          2, 'win_loss'),
    ('その他',                  '1v1',          2, 'win_loss');
```
...
申告・承認・差し戻しなどの状態変化は、ルーム参加者全員の画面に即時反映する必要がある。  
既存の憑依華ロビーの WebSocket エンドポイント（`/ws/hyouibana`）と同様の仕組みを流用し、大会用エンドポイント `/ws/tournament` を新設する。

### ブロードキャストイベント一覧

| イベント | 送信タイミング | 受信側の挙動 |
| :--- | :--- | :--- |
| `match.reported` | 勝者が申告した瞬間 | **全参加者**の該当試合カードを「📋 承認待ち」に切り替え |
| `match.approved` | Staffが承認した瞬間 | **全参加者**の試合カードを「✅ 確定」に切り替え。ブラケット/ランキングを更新 |
| `match.rejected` | Staffが差し戻した瞬間 | **申告者**のカードを「再申告可」に戻す。全参加者に「差し戻し」バッジ表示 |
| `score.reported` | multiplayer で1人が順位申告した瞬間 | **全参加者**の申告状況一覧（誰が申告済みか）をリアルタイム更新 |
| `match.disputed` | 順位重複検出時 | **全参加者**に「⚠️ 矛盾あり」バッジ表示。Staffへ通知 |

> [!NOTE]
> ページリロードなしで状態が切り替わることで、「申告したのに相手が気づいていない」「いつ承認されたかわからない」といった運営コストを削減する。
...
- 試合開始時に「結果画面のスクリーンショットを保存してください」の警告をダッシュボードに表示する。disputed 発生時の証拠提出を想定。

### 6.3 multiplayer モード（マリオカート等）
...
- 各レース開始時に「結果画面のスクリーンショットを保存してください」の警告をダッシュボードに表示する。disputed 発生時の証拠提出を想定。

---

## 7. フロントエンド UI/UX

### 7.1 大会作成フォーム（既存ロビー作成フォームを拡張）

```
- ゲームタイトル: ドロップダウン選択（game_titles から動的取得）
- ロビー名: テキスト入力
- 合言葉: テキスト入力（任意。空欄の場合は合言葉なしとして扱う）
- 説明書き: テキストエリア
...
> [!NOTE]
> 本フローは憑依華ロビーシステム（`feature/secure-lobby`）にも後から適用することを想定した設計とする。

**部屋作成時:**
- 合言葉フィールドは任意入力
- 空欄で作成した場合: `passcode = NULL` として保存。参加時の合言葉入力をスキップ
- 入力して作成した場合: 合言葉あり部屋として保存

**部屋参加時:**
- 「参加する」ボタンを押した後にモーダルを表示（参加ボタン押下前の入力欄は設けない）
  - 合言葉ありの部屋: モーダルに合言葉入力欄を表示 → 照合成功で参加完了
  - 合言葉なしの部屋: モーダルなし、ボタン押下で即参加

**バリデーション:**
- 誤った合言葉を入力した場合はモーダル内にエラー表示。入力欄はクリアしない（再入力しやすくする）
- 3回連続失敗で一時的に参加をブロックし、Staffへ通知（ブルートフォース対策）
...
> [!WARNING]
> 試合・レース開始後、申告フォームの上部に以下の警告を常時表示する。  
> 「📸 **結果画面のスクリーンショットを必ず保存してください。** 申告内容に異議が生じた場合、証拠として提出を求めることがあります。」
...
- リーグ方式の追加を行うか
- チーム戦の場合、どのように分けるか
- 決勝戦がゲーム内で別れた場合、どのように勝敗処理を行うか
- 異議申し立てが出た場合の全体通知および処理
- カービィのエアライダーのシティトライアルでのゲーム固有のルール
