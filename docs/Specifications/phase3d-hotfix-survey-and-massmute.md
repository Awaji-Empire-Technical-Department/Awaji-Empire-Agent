...
- **経緯**: Phase 4 に計画していた「フェーズ 4.1: 緊急バグ修正」を、本番稼働中の影響度を考慮して Phase 3-D として前倒し実施する。

---

## 1. 目的

Phase 3 で稼働した Rust Bridge を経由した本番環境において、以下の 2 つの問題がすでに発生している。
...
- [ ] **[Survey]** 本番 DB で `survey_responses` テーブルの中身と `submitted_at` の型を確認
...
- [ ] **E2E**: 本番（またはテスト）環境でアンケート回答 → ダッシュボード確認を実施
