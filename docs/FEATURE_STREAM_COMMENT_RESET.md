...
具体的には、Voice Keeper が**毎月1日**の配信終了後に寝落ち集計報告を `#配信コメント` へ投稿した直後にリセットを実行する。これにより配信中のリセットによる混乱を防ぐ。1日に配信がない・報告が失敗した場合は**翌2日 06:00 JST のフォールバック cron のみ**が補完する。2日を過ぎた場合はスラッシュコマンドによる手動実行のみ対応（月初め以外のリセットは想定しない）。
...
| ID | 要件 |
|----|------|
| FR-01 | **毎月1日**に Voice Keeper の寝落ち集計報告を `#配信コメント` で検知した直後、`#配信コメント` チャンネルを削除し同名・同カテゴリ・同ポジションで再作成する（主トリガー） |
| FR-01b | 主トリガーが発火しなかった場合（1日に配信なし・報告失敗）のフォールバックとして、**毎月2日 06:00 JST のみ** 当月リセット済みかを確認し、未実施であれば実行する。2日を過ぎた場合はスラッシュコマンドによる手動リセットのみ対応 |
...
#### フォールバック cron（1日に配信がなかった場合のみ・毎月2日限定）

```python
    @tasks.loop(hours=24)
    async def fallback_reset(self):
        """1日に VoiceKeeper 報告がなかった場合のみ、2日 06:00 JST に補完する。
        月初め以外ではリセットを想定しないため、day==2 の条件で厳密に制限する。
        """
        now = datetime.now(JST)
        if now.day != 2 or now.hour != FALLBACK_HOUR_JST:
            return  # 毎月2日 06:00 JST 以外は何もしない
        await self._try_monthly_reset(triggered_by="fallback_scheduler")
```

> **Note**: 2日を過ぎてリセットが実行されなかった場合は、
...
[Voice Keeper が #配信コメント に寝落ち集計報告を投稿]
  ...
  _execute_reset() → 共通リセットロジックへ ─────────────┐
                                                         │
[フォールバック cron: 毎月2日 06:00 JST 限定]             │
          │  tasks.loop(hours=24)                        │
          ▼                                              │
  now.day == 2 かつ now.hour == 06 か？                  │
       No  → 無視（月初め以外はリセット対象外）            │
       Yes ↓                                             │
          ▼                                              │
...
| フォールバック cron：2日 06:00 かつ未リセット | `day==2, hour==6` のとき `_try_monthly_reset("fallback_scheduler")` が呼ばれること |
| フォールバック cron：2日以外 | `day != 2` の場合は何もしないこと（月初め以外はリセット対象外） |
| フォールバック cron：2日だが時刻不一致 | `day==2` でも `hour != 6` の場合は実行されないこと |
| フォールバック cron：2日かつリセット済み | 1日に VoiceKeeper 報告でリセット済みの場合、2日 06:00 のフォールバックは何もしないこと |
...
| `FALLBACK_HOUR_JST` | `int` | `6` | フォールバック cron の実行時刻（JST）。毎月2日のみ有効 |
