...
### Discord API への依存を減らす意図

称号の状態管理をWebアプリ側（DB）に集約することで、Discord API の呼び出しを「装備切替時のロール付け替え1回」に絞り込める。  
これは以下の理由で重要な設計判断である。

- **レートリミット回避**: Discord API には1秒あたりのリクエスト上限がある。称号の取得・表示・検索をすべて Webアプリ側で完結させることで、API 呼び出しをイベント駆動（装備変更時のみ）に限定できる
- **可用性の分離**: Discord の障害・メンテナンス時でも称号の閲覧・管理・付与がWebアプリ単体で継続できる
