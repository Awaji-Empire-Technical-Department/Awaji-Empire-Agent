...
| 日付 | 変更内容 |
|------|---------|
| 2026-04-20 | リセット実行日を毎月1日→**毎月20日**、フォールバック cron を毎月2日→**毎月21日** 06:00 JST に変更。運用スケジュール調整による変更で、アーキテクチャに変更なし。
...
* **主トリガー**: `on_message` で VoiceKeeper が `#配信コメント` に投稿する寝落ち集計報告（キーワード `寝落ち` を含む Bot メッセージ）を検知 — 毎月 20 日のみ発火。
* **フォールバック**: `tasks.loop(hours=24)` で毎月 21 日 06:00 JST に未リセットなら補完実行。21 日を過ぎた場合はスラッシュコマンドのみ。
