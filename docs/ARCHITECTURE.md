...
この機能は、指定されたチャンネルの通知設定を **@everyone ロールに対して強制的に上書き**することで、サーバー全体の通知を抑制（実質的なミュート）します。この処理は、Bot 起動時や毎日指定された固定時刻に実行されます。

Bot の再起動による状態の消失を防ぐため、堅牢なログとエラー通知機能が実装されています。
...
1.  **トリガー (Trigger)**:
    * **Startup**: Bot 起動完了後、`bot.py` の `on_ready` から `MassMuteCog.execute_mute_logic` を直接呼び出し、即座に実行されます。
    * **Daily Task**: `discord.ext.tasks` により、毎日指定された固定時刻（UTC 0:00, 8:00, 16:00）に自動実行されます。
2.  **サーバー探索**: Bot が参加している最初のサーバー（淡路帝国サーバー）を取得します。
3.  **チャンネル探索**: `config.MUTE_CHANNEL_NAMES` リストに基づき、対象チャンネルを名前で探索します。
4.  **権限上書き**:
    * 探索された各チャンネルに対し、`@everyone` ロールの権限を **`MUTE_OVERWRITE`**（`mention_everyone=False`, `manage_webhooks=False` など）で上書きし、通知を強制的に抑制します。
5.  **ログ/通知**:
    * 権限設定に成功した場合、管理者へ実行完了 DM を送信。
    * チャンネルが見つからない、または Bot に**「権限の管理」権限がない**ために失敗した場合、その詳細を記載したエラー DM を管理者へ送信します。

#### 2.3.2 使用される設定値 (Config)

この機能は、外部から以下の設定値に依存します。

| 設定名 | ファイル | 形式 | 用途 |
| :--- | :--- | :--- | :--- |
| `ADMIN_USER_ID` | `config.py` | `str` (実行時に `int` に変換) | 処理結果やエラー通知 DM の送信先となる管理者 ID。 |
| `MUTE_CHANNEL_NAMES` | `config.py` | `List[str]` | 通知を抑制する対象チャンネルの**名前**リスト。 |
...
| 権限 | 適用範囲 | 理由 |
| :--- | :--- | :--- |
| **権限の管理** | サーバー全体 または 対象チャンネル | `@everyone` ロールのチャンネル権限設定をBotが**上書き変更**（通知オフに強制設定）するために必須です。 |
