...
## LESSON-006: Jinja2 で `enumerate()` は使えない → `loop.index` / `loop.first` を使う
...
`lounge.html` / `tournament.html` で順位表を表示するために `{% for i, s in enumerate(standings) %}` と書いた結果、`UndefinedError: 'enumerate' is undefined` が発生して 500 エラーになった。LESSON-005 と同根だが `enumerate()` は見落としやすいため別途記録する。

```jinja2
{# NG: enumerate はJinja2に存在しない #}
{% for i, s in enumerate(standings) %}
  <td>{{ i + 1 }}位</td>
  <tr class="{{ 'first' if i == 0 }}">
{% endfor %}
...
| `loop.index` | 1始まりの連番 |
| `loop.index0` | 0始まりの連番 |
| `loop.first` | 最初の要素なら `True` |
| `loop.last` | 最後の要素なら `True` |

#### ルール

> **Jinja2 でインデックス付きループを書くときは `enumerate()` を使わず `loop.index` / `loop.index0` / `loop.first` を使うこと。**

---

## LESSON-007: deploy.yml が database_bridge を restart せず、Rustビルドが反映されない

- **発見日**: 2026-06-14
- **症状**: `Bridge API Error (GET .../surveys/10/collaborators): 404`（新規ルートが 404）
- **修正**: `.github/workflows/deploy.yml` の Restart Services に
  `sudo systemctl restart database_bridge.service` を追加（両ジョブ）

### 何が起きたか

CI/CD は `cargo build --release` で新バイナリを作り rsync するが、
`Restart Services` ステップは `discord_bot` / `discord_webapp` しか restart せず、
`setup-systemd.sh` も copy/enable/daemon-reload のみで restart を行わない。
結果、**動作中の database_bridge プロセスが古いバイナリのまま**となり、
新しく追加した API ルートが 404 を返した。

### 影響

- Rust 側の変更（新ルート・スキーマ対応）は deploy しても無反映。
  手動再起動かサーバ再起動でしか反映されず、長く潜在化していた。
